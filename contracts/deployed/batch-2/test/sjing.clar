...
(define-private (stx-transfer-to (ustx uint) (to principal) (memo (optional (buff 34))))
  (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dmg transfer ustx tx-sender to memo))
  ...
  (let ((id (var-get next-id)))
    (asserts! (map-insert swaps id
        ...
        (print 
      {
        ...
        open: true,
        in_contract: 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dmg,
        in_amount: ustx,
        in_decimals: (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dmg get-decimals) ERR_FT_FAILURE),
        out_contract: 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.cha, 
        out-amount: amount,
        out-decimals: (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.cha get-decimals) ERR_FT_FAILURE),
      }
    )
    (var-set next-id (+ id u1))
    (try! (contract-call? .sfire hold-fees ustx))
...
(define-public (cancel (id uint))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    (ustx (get ustx swap)))
      ...
      (try! (contract-call? .sfire release-fees ustx)) 
    (print 
      {
        type: "cancel",
        swap_type: "STX-FT",
...
(define-public (submit-swap (id uint))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    ...
    (ft-amount (get amount swap)))
      ...
      (try! (contract-call? .sfire pay-fees ustx))
      (print 
        {
            type: "swap",
            swap_type: "STX-FT",
      ...
      (match (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.cha transfer
...
;; (err u1) -- sender does not have enough balance to transfer u1000
;; (err u2) -- sender and recipient are the same principal u2000
